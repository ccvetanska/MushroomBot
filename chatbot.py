...
class MushroomBot:
  ...
  def welcome(self):
    print(self.format_message("Здравей, аз съм твоят помощник за разпознаване на гъби. 🍄 Ще ти задам няколко въпроса за гъбата, която искаш да идентифицираш и ще се постарая да намеря най-близкото съвпадение. Ако не знаеш отговора на някой върпос, остави полето празно. Нека да започваме!"))
